-- ============================================================
-- BLINDERS V4 — FIX CATEGORY + BINGO 100 + ADMIN ACTION + PERFORMANCE
--
-- Corrige:
...
--
-- Resultado esperado:
-- BLINDERS_NETLIFY_EVOLUTION_FULL_V4_CORRIGIDO_OK
-- ============================================================
...
-- ------------------------------------------------------------
-- Fix shop_items SEM usar id antes de existir + category obrigatório
-- ------------------------------------------------------------
...
set
  ...
  category = coalesce(nullif(category,''), nullif(item_type,''), 'profile'),
  description = coalesce(description, ''),
  item_type = coalesce(nullif(item_type,''), nullif(category,''), 'badge'),
...
alter table public.shop_items alter column category set default 'profile';

create index if not exists ix_shop_items_item_key_safe on public.shop_items(item_key);
create index if not exists ix_shop_items_active_safe on public.shop_items(active, category, rarity);

insert into public.shop_items(
  item_key,title,name,category,description,item_type,rarity,
  price_units,price,stock,active,metadata,data,created_at,updated_at
)
select
  v.item_key,v.title,v.name,v.category,v.description,v.item_type,v.rarity,
  v.price_units,v.price_units,0,true,'{}'::jsonb,'{}'::jsonb,now(),now()
from (values
  ('silver_frame','Moldura Prata','Moldura Prata','profile','Moldura segura para perfil','frame','rare',1500000000000::numeric),
  ('vip_title','Título VIP','Título VIP','profile','Título visual de perfil','title','rare',2000000000000::numeric),
  ('event_badge','Badge Evento','Badge Evento','event','Insígnia especial','badge','epic',3000000000000::numeric)
) as v(item_key,title,name,category,description,item_type,rarity,price_units)
where not exists (
...
-- ============================================================
-- BLINDERS V5 — ADMIN SUPABASE FIX
-- Corrige "Supabase não configurado no Netlify" mantendo segurança:
-- - admin actions funcionam via Netlify Function com anon fallback
-- - se Function falhar, frontend usa RPC direta
-- - RPC valida token e cargo admin/owner
-- - service_role NÃO é necessária para essas ações
-- ============================================================
...
create index if not exists ix_v5_sessions_token on public.app_member_sessions(token);
...
drop function if exists public.app_admin_secure_action(text,text,jsonb);
create or replace function public.app_admin_secure_action(
  p_token text,
  p_action text,
...
begin
  ...
  values(
    ...
    coalesce(p_payload->>'target', p_payload->>'code', ''),
    coalesce(p_payload,'{}'::jsonb)
  );

  return jsonb_build_object(
    'ok',true,
    'message','Ação executada e registrada.',
    'action',p_action,
    'admin',admin.nick,
    'at',now()
  );
...
drop function if exists public.app_admin_confirm_deposit(text,text,text,text);
create or replace function public.app_admin_confirm_deposit(
  p_token text,
  p_code text,
  p_amount text,
  p_note text default ''
...
begin
  ...
  if amount <= 0 then
    raise exception 'Valor do depósito precisa ser maior que zero.';
  end if;
  ...
  values(
    ...
    'confirm_deposit',
    coalesce(p_code,''),
    jsonb_build_object('amount',amount,'amountLabel',public.v25_format(amount),'note',p_note)
  );
  ...
  return jsonb_build_object(
    ...
    'code',p_code,
    'amountLabel',public.v25_format(amount)
  );
end;
$$;

drop function if exists public.app_admin_confirm_withdraw(text,text,text);
create or replace function public.app_admin_confirm_withdraw(
  p_token text,
  p_code text,
  p_note text default ''
)
...
begin
  ...
  insert into public.admin_audit_logs(admin_id,action,target,payload)
  values(admin.id,'confirm_withdraw',coalesce(p_code,''),jsonb_build_object('note',p_note));

  return jsonb_build_object(
    'ok',true,
    'message','Saque marcado como conferido.',
    'code',p_code
  );
end;
$$;

drop function if exists public.app_health_public();
create or replace function public.app_health_public()
returns jsonb
language plpgsql
security definer
...
begin
  return jsonb_build_object(
    'ok',true,
    'database','online',
    'membersTable', to_regclass('public.app_members') is not null,
    'sessionsTable', to_regclass('public.app_member_sessions') is not null,
    'adminAuditTable', to_regclass('public.admin_audit_logs') is not null,
    'time',now()
  );
end;
$$;
...
grant execute on function public.app_admin_confirm_deposit(text,text,text,text) to anon, authenticated;
grant execute on function public.app_admin_confirm_withdraw(text,text,text) to anon, authenticated;
grant execute on function public.app_health_public() to anon, authenticated;
...
select 'BLINDERS_NETLIFY_EVOLUTION_FULL_V5_ADMIN_SUPABASE_FIX_OK' as status;
